...
- **TODO Formulaire — Option 1 choisie puis mise en pause (2026-09-04)** : test phase — pas de domaine valide → `FORM_ENDPOINT=""` `app.js:294` (simulation seule, toast “simulation” `app.js:335`). Code FormSubmit prêt (`https://formsubmit.co/ajax/contact@kids-training-reunion.re` + hidden `_captcha/_template/_subject/_autoresponse` dans `index.html:664`) retiré temporairement, à remettre quand domaine actif. 1er vrai envoi → valider email confirmation FormSubmit sur `contact@kids-training-reunion.re`.
